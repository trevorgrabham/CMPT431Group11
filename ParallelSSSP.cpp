...
void findShortestPaths(int id, int nThreads, Graph const &g, std::vector<uintV> &distance, std::vector<uintV> &path,
  ...
  // keep waiting on barrier for threads that have not exite yet
  // final thread guaranteed to exit after all nodes have been processed
  done++;
  while (done < nThreads) b.wait();
  b.open(); // all threads are done, open the barrier to release all threads
}
